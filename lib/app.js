// well organized, well-named and RESTful routes!
...
app.get('/nasa', async(req, res) => {
  ...
  // seems like this should be behind an `/api` route, since you're exposing your API key
  try {
...
app.get('/api/note/:id', async(req, res) => {
  ...
  try {
    const data = await client.query(`
      ...
      WHERE users.id=$1
      AND notes.id=$2`, 
    // awesome use of two ids to make a query!
